...
ladder was the same for both classes anyway, so there was no way to say "media
over raw TCP from this network, control traffic through Cloudflare" — which
matters when a residential-looking IP keeps bulk transfers off the anti-spam
radar, or when only one class of upstream works on your network.
